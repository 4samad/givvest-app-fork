	...
	AccessControl
{
	 ...
	 */
	constructor(address initialOwner) ERC721("GivvestCause", "GCOS") {
	...
	function safeMint(
		...
		//assigns tokenCount to tokenId before incrementing
	...
	function raiseAmount(
		uint256 tokenId,
		uint256 amount
	) public onlyRole(GIVVEST_ROLE) {
		Cause storage cause = causes[tokenId];
		require(cause.isActive, "Cause is not actively fundraising");
		require(
			cause.amountRaised + amount <= cause.amountNeeded,
			"Amount is more than needed"
		);
	...
	function toggleCauseState(uint256 tokenId) public {
		Cause storage cause = causes[tokenId];
		require(
			cause.isActive || cause.amountRaised < cause.amountNeeded,
			"Fundraising is already complete"
		);
		require(
			cause.fundraiser == msg.sender ||
	 ...
	 */
	function grantGivvestRole(
		address givvestContract
	) public onlyRole(DEFAULT_ADMIN_ROLE) {
